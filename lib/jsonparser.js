...
phssr.makeHardCodedStyleTag = function(reporterOptions){
    ...
    styleTag+= ".env-col{min-width:130px;}";
...
phssr.makeHTMLPage = function(tableHtml, reporterOptions){
    ...
    staticHTMLContentprefix +=  "<th class='env-col'>Environment</th>";
...
function generateHTML(data){
    ...
    str +=  '<td class="env-col">' + '<b>OS:</b> ' + data.os + '<br/><b>Browser:</b> ' + data.browser.name + ' ' + data.browser.version + '</td>';
    if (data.startTime && data.endTime) {
        str += '<td class="time-col"><b>Start time: </b><br/><span style="font-size:10">' + new Date(data.startTime).toISOString() + '</span>';
        str += '<br/><b>End time: </b><br/><span style="font-size:10">' + new Date(data.endTime).toISOString() + '</span>';
        str += '<br/><b>Total time: </b>' + (Math.round((data.endTime - data.startTime) / 100) / 10).toString() + 's';
        str += '</td>';
    }
    else {
        str += '<td class="time-col"><b>No data</b></td>';
    }
