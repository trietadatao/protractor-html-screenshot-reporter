...
function buildSummaryReport() {
    ...
    htmlCode += "<h2 align='center'>Summary Report</h2>" +
        "<table>" +
        "<tr class='header' align='center'><th>Test suite</th><th>Total cases</th><th>Pass</th><th onclick=\"javascript:(function(){$('tr:has(td:contains(true))').toggle()})()\">Fail</th></tr>";
    for (var key in reportInfo) {
        ...
        passPercent = (report.passCount / report.total * 100).toFixed(2);
    ...
    //Final total row
    passPercent = (totalPass / totalTest * 100).toFixed(2);
    if (suiteCount > 1) {
